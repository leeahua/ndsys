...
<head>
    ...
    <script type="application/javascript" src="/static/js/pigweight.js"></script>
</head>

<body style="margin: 1px;">
    <table id="dg" title="猪肉重量采集信息维护列表" class="easyui-datagrid"
    ...
    <div id="dlg" class="easyui-dialog" style="width:400px;height:400px;padding:10px 20px"
        ...
        <form id="fm" method="post" >
            ...
            <div style="margin-bottom:10px">
                <input class="easyui-textbox" name="pigWeight" style="width:100%" data-options="label:'重量:',required:true">
            </div>
...
<script>
    ...
    function editUser(){
        ...
        if (row){
            ...
            $('#fm').form('load',row);
            url = '/pigweight/update?id='+row.id;
        }
    }
    function saveUser(){
        $.ajax({
    ...
    function destroyUser(){
        ...
        if (row){
            $.messager.confirm('警告','您确定要删除这条记录么?',function(r){
                if (r){
                    $.post('/pigweight/delete',{id:row.id},function(result){
                        alert(result.msg);
                        if (result.code=="00"){
                            $('#dg').datagrid('reload');	// reload the user data
                        }
