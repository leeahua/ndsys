 ...
 */
public class SerialCOM22Listener implements SerialPortEventListener {
    ...
    private static String LISTENER_PORT = "COM2";

    //2.构造函数：
    //实现初始化动作：获取串口COM21、打开串口、获取串口输入流对象、为串口添加事件监听对象
    public SerialCOM22Listener(){
        try {
            //获取串口、打开窗串口、获取串口的输入流。
            com21 = CommPortIdentifier.getPortIdentifier(LISTENER_PORT);
            serialCom21 = (SerialPort) com21.open("Com2EventListener", 38400);
            ...
            try {
                ...
                while(availableBytes > 0){
                    ...
                    if(dbPigWeightList.size()==0){
                        ...
                        log.info("update----batchno:{}------->",new BigDecimal(sb.toString().split("@")[0]));
                        PigWeight dbPig = dbPigWeightList.get(0);
                        dbPig.setPigWeight(new BigDecimal(sb.toString().split("@")[1]));
